...
- add: malformed GPX/KML/GML gets one conservative repair-and-retry — real-world exporters systematically ship XML with unescaped text (`&` inside URLs, `<` inside values like `<25%`); when GDAL rejects such a file, the scan retries once on a repaired temp copy (bare `&` → `&amp;`, bare `<` → `&lt;`, the original file untouched) and logs `repaired malformed XML (N substitutions)`; a file that stays unreadable keeps a single-line `✗`
- fix: a vector scan no longer crashes with a raw `KeyError: JsonType(extension<arrow.json>)` on JSON-typed property columns — pyogrio maps nested GeoJSON properties (valid per spec, also produced by lenient GDAL reads of nonstandard files) to the canonical `arrow.json` Arrow extension type, which is now cast to its string storage so the values stay catalogable as text
- change: a multi-layer GML/KML scan now warns visibly, naming the skipped layers (`4 layers; scanned 'X' — skipped: 'Y', 'Z'`) instead of hiding the loss in a debug line; GPX keeps the debug line (its fixed layers are facets of one recording, the populated one is scanned)
