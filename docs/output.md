...
When `app_path/data/db-ui` exists, it is loaded automatically as the last metadata source before export. This lets local app edits override scanned metadata and configured `metadata_path` sources without adding `data/db-ui` to the configuration.
