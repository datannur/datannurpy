...
The exported `data/db` is always disposable — rewritten on every run, so never edit it by hand. Keep your edits in the **source of truth**, and where that lives defines the workflow:

- **Artefact** — the source of truth is external files (`metadata_path`, scanned folders); the output is gitignored and regenerated by CI. Pair with `refresh: true` and `on_scan_error: fail` / `on_metadata_error: fail` so a broken run fails red instead of publishing a degraded catalog. This is the GitHub Pages template flow.
- **Installation** — the app lives in place long-term; you edit inside it (`db-ui`) and refresh bundled files with `update_app: true`. Local app state under `data/` is preserved across runs (see [Incremental scan](#incremental-scan)).

The two combine — an installation can still merge external metadata (`db-source`), as `examples/demo_editorial.yml` does. Only one rule is fixed: `data/db` is output, edits belong in `metadata_path` files or the in-app `db-ui`.
